...
def create_icdar_train_dataset(img_path, gt_path, batch_size=32, repeat_num=10, 
    ...
    dataset = ds.GeneratorDataset(source=datasetV2(img_path, gt_path), column_names=['image', 'annotation', 'label'],
    ...
    #normalize_op = C.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
...
def load_annoataion(path):
    ...
    for line in lines:
        ...
        labels.append(label)
    return np.array(vertices, dtype=np.float32), np.array(labels, dtype=np.float32)
...
class datasetV2():
    def __init__(self, img_path, gt_path):
        ...
        self.data = [None] * len(self.img_files)
    ...
    def __getitem__(self, index):
        if self.data[index] is not None:
            img, annos, labels = self.data[index]
            return img, annos, labels
        else:
            img = cv2.imread(self.img_files[index])
            annos, labels = load_annoataion(self.gt_files[index])
            self.data[index] = [img, annos, labels]
            return img, annos, labels
...
def benchmark():
    ...
    dataset = create_icdar_train_dataset(train_img_path, train_gt_path, 
