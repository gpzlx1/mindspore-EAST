...
from mindspore.dataset.transforms.vision import py_transforms  
...
def get_score_geo(img, vertices, labels, scale, length):
    ...
    score_map = np.transpose(score_map, perm)
    geo_map = np.transpose(geo_map, perm)
    ignored_map = np.transpose(ignored_map, perm)
